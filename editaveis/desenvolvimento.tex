...
Alternativa de solução: adquirir um quadro comum no comércio e realizar as adaptações necessárias.
	...
	\subsection{Adaptação do QuadrConstrução}
...
\newpage
	...
	Para a caixa das baterias e placas eletrônicas foi escolhido a madeira MDF (Medium Density Fiberboard), como material. O MDF foi escolhido devido ao seu baixo custo, facilidade de manuseio e por apresentar especificações mecânicas suficientes para o projeto. O MDF possui baixa condutividade térmica, podendo atuar como isolante térmico e apresenta resistência a variação de temperatura, não conduz eletricidade e tem uma boa resistência mecânica e elástica.
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.7\textwidth]{box_frontal.png}
			\caption{Visão frontal da caixa}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.7\textwidth]{box_inferior.png}
			\caption{Visão inferior da caixa }
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.7\textwidth]{box_lesquerda.png}
			\caption{Visão lateral esquerda da caixa }
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.7\textwidth]{box_ldireita.png}
			\caption{Visão lateral direita da caixa}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.7\textwidth]{box_traseira.png}
			\caption{Visão traseira da caixa}
		...
		\subsection{Processo de fabricação da caixa}
		Foram compradas placas de 12mm de espessura de MDF cru, 100 parafusos de 3,5x30mm, 30 parafusos de 3x12mm, 2 lixas para madeira Nº 120, 2 dobradiças pequenas, 1 trinco pequeno, 6 abraçadeiras de 1’’,\textbf{ outra madeira} de 6mm.
		O processo de fabricação foi dividido em duas etapas, a caixa das baterias e a caixa principal. As medidas foram feitas a partir do desenho do CAD. 
		A caixa das baterias foi produzida a partir de cinco peças, sendo elas, uma base, duas laterais e uma parte frontal e outra traseira. As partes laterais e a base são compostas por MDF de 12 mm e as partes frontal e traseira foram feitas com\textbf{ outra madeira} de 6mm. Para o corte das madeiras foram utilizados uma serra de mesa e uma serra tico-tico. Para a fixação das peças foram utilizados uma furadeira/parafusadeira. O processo de montagem consistiu em utilizar a furadeira com uma broca de 2,5mm para perfurar um furo guia nas madeiras e a partir dessa guia executar o parafusamento direto. 
		A caixa principal foi produzida a partir de oito peças, sendo elas, uma base, uma parte lateral maior, uma lateral menor, uma porta, uma parte traseira, uma parte frontal e duas partes superiores. As duas partes superiores são compostas com \textbf{outra madeira} de 6mm e as outras partes foram feitas com o MDF de 12mm. O processo de fabricação da caixa principal foi o mesmo da caixa das baterias. 
		A pintura da parte externa da caixa principal foi feita após sua montagem. Para a pintura, primeiramente foi lixada toda a parte externa a ser pintada, após ser lixada foi feita a pintura direta com uma tinta spray preta. A aplicação da tinta foi feita de forma direta com aplicações pausadas. O mesmo foi feito para toda a bicicleta.
		...
		\subsection{Estabilidade da Bike}
		...
		Observou-se uma mudança no centro de massa referente a adição das baterias e,posteriormente, a presença de um cliente de massa já especificada e também comentado no escopo. O comportamento dessas forças foram ilustradas anteriormente nas Figuras\ref{img:esq_forca_sem_frenagem}, \ref{img:esq_forca_com_frenagem_suave} e \ref{img:esq_forca_com_frenagem_brusca}.
